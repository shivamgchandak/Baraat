/**
 * Driver trip-status transitions. Each transition updates driver state and
 * recomputes predictedFreeAt / predictedFreeLocation — the engine's inputs
 * for "who is free, when, and where".
 */
...
export async function transitionTrip(
  ...
  switch (next) {
    ...
    case TripStatus.REJECTED: {
      // Re-queue guests for reassignment; driver becomes idle again.
      await prisma.$transaction([
        prisma.trip.update({ where: { id: tripId }, data: { status: next } }),
        prisma.guest.updateMany({
          where: { id: { in: guestIds } },
          data: { status: GuestStatus.WAITING }, // waitingSince preserved -> keeps queue priority
        }),
        prisma.driver.update({
          where: { id: driverId },
          data: {
    ...
    case TripStatus.ARRIVED_DROP: {
      ...
      // auto-complete in the same call path
